...
var src_utils_mock = require('../../../src/utils/mock.js');
var packages_elementPlus_form_config = require('./config.js');
var menu_vue_vue_type_script_lang = require('./menu2.js');
var index_vue_vue_type_script_lang = require('../../core/components/form/index2.js');

var script = src_core_create.default({
  name: "form",
  mixins: [packages_core_common_init.default("form")],
  emits: [
    "update:modelValue",
    "update:status",
    "reset-change",
  ...
  components: {
    formTemp: index_vue_vue_type_script_lang.default,
    formMenu: menu_vue_vue_type_script_lang.default,
  },
  data() {
    return {
      config: packages_elementPlus_form_config.default,
      activeName: "",
      allDisabled: false,
      tableOption: {},
      form: {},
    ...
    columnOption() {
      const processColumnDetails = (list) => {
        list.forEach((groupItem, groupIndex) => {
          groupItem.column = src_utils_util.getColumn(groupItem.column) || [];

          groupItem.column.forEach((column, columnIndex) => {
            if (column.display !== false && !this.isMobile) {
              column = src_core_dataformat.calcCount(
                column,
          ...
          groupItem.column = src_core_dataformat.calcCascader(groupItem.column);
      ...
      const mainColumn = src_utils_util.getColumn(tableOption.column);

      // 处理分组配置
      const processedGroups = (tableOption.group || []).map((groupItem) => ({
        ...groupItem,
        column: src_utils_util.getColumn(groupItem.column),
      }));
  ...
  methods: {
    getComponent: src_core_dataformat.getComponent,
      ...
      this.propOption.forEach((column) => {
        ...
        if (bind) {
          let formProp = this.$watch("form." + prop, (n, o) => {
            src_utils_util.setAsVal(this.form, bind, n);
          });
          ...
          this.form[prop] = src_utils_util.getAsVal(this.form, bind);
        }
        if (control) {
          const callback = () => {
    ...
    setLabel() {
      if (this.tableOption.filterNull === true) {
        this.form = src_utils_util.filterParams(this.form, [""], false);
      }
      if (this.tableOption.filterDic == true) {
        ...
        this.propOption.forEach((column) => {
          ...
          result = src_core_detail.detail(this.form, column, this.tableOption, DIC);
          if (result) {
        ...
        cascader.forEach((item) => {
          ...
          if (this.formList.includes(str)) {
            ...
            cascader.forEach((ele) => {
              this.form[ele] = src_utils_util.blankVal(this.form[ele]);
              this.DIC[ele] = [];
            });
          }
          /**
    ...
    handleMock() {
      ...
      this.columnOption.forEach((column) => {
        const form = src_utils_mock.default(column.column, this.DIC, this.form, this.isMock);
    ...
    resetForm(reset = true) {
      if (reset) {
        ...
        this.form = src_utils_util.clearVal(
          this.form,
          propList,
    ...
    submit() {
      this.validate((valid, hide, msg) => {
        if (valid) {
          this.$emit("submit", src_utils_util.filterParams(this.form, ["$"]), this.hide);
        } else {
          this.$emit("error", msg);
        }
      });
