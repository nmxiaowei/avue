...
var src_plugin_watermark_index = require('../../../src/plugin/watermark/index.js');
var src_plugin_qiniu_index = require('../../../src/plugin/qiniu/index.js');
var src_plugin_ali_index = require('../../../src/plugin/ali/index.js');
...
var script = src_core_create.default({
  ...
  methods: {
    ...
    isMediaType(url) {
      return src_utils_util.isMediaType(url, this.fileType);
    },
    setSort() {
      if (!window.Sortable) {
    ...
    httpUpload(config) {
      ...
      const done = () => {
        ...
        const handleUploadResult = (res) => {
          ...
          this.res = src_utils_util.getAsVal(this.isAliOss ? res : res.data, this.resKey);
        ...
        const uploadToCos = () => {
          if (!window.COS) {
            src_core_packages.default.logs("COS");
            hide();
            return;
          }
        ...
        const uploadToQiniu = () => {
          if (!window.CryptoJS) {
            src_core_packages.default.logs("CryptoJS");
            hide();
            return;
          }
          oss_config = this.qiniu || this.$AVUE.qiniu;
          const token = src_plugin_qiniu_index.getToken(oss_config.AK, oss_config.SK, {
        ...
        const uploadToAliOss = () => {
          if (!window.OSS) {
            src_core_packages.default.logs("AliOSS");
            hide();
            return;
          }
          oss_config = this.ali || this.$AVUE.ali;
          client = src_plugin_ali_index.getClient(oss_config);
      ...
      if (src_utils_util.isMediaType(file.name) != "img") {
        done();
      } else {
        //处理水印图片
        const canvasDone = () => {
          if (!this.validatenull(this.canvasOption)) {
            src_plugin_watermark_index.detailImg(file, this.canvasOption).then((res) => {
              file = res;
              done();
            });
          } else {
