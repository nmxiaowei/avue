...
var script = src_core_create.default({
  ...
  methods: {
    ...
    toNumber (value, fallback = 0) {
      ...
      return Number.isFinite(number) ? src_utils_util.getFixed(number) : fallback;
    },
    ...
    snap (value) {
      ...
      return src_utils_util.getFixed(grid > 0 ? Math.round(value / grid) * grid : value);
    },
    clampSize (value, dimension) {
      ...
      return src_utils_util.getFixed(Math.max(min, Number.isFinite(max) && max > 0 ? Math.min(value, max) : value));
    },
    constrainSizeToBounds (value, dimension) {
      const bounds = this.getBounds();
      if (!bounds) return src_utils_util.getFixed(value);
      const limit = dimension === 'width'
        ? Math.max(0, bounds.right - bounds.left)
        : Math.max(0, bounds.bottom - bounds.top);
      return src_utils_util.getFixed(Math.min(value, limit));
    },
    normalizeBox () {
    ...
    constrainPosition (left, top) {
      const bounds = this.getBounds();
      if (!bounds) return { left: src_utils_util.getFixed(left), top: src_utils_util.getFixed(top) };
      return {
        left: src_utils_util.getFixed(Math.min(Math.max(left, bounds.left), Math.max(bounds.left, bounds.right - this.baseWidth))),
        top: src_utils_util.getFixed(Math.min(Math.max(top, bounds.top), Math.max(bounds.top, bounds.bottom - this.baseHeight)))
      };
    },
    constrainResize (left, top, width, height, sides) {
      ...
      return {
        left: src_utils_util.getFixed(left),
        top: src_utils_util.getFixed(top),
        width,
        height
      };
    },
    emitMove (delta) {
      this.$emit('move', { index: this.index, left: src_utils_util.getFixed(delta.x), top: src_utils_util.getFixed(delta.y) });
