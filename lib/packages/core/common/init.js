...
function init (name) {
    return {
        ...
        computed: {
            rowDicColumns() {
                ...
                const columns = name === 'crud'
                    ? this.propOption.filter(src_core_dic.isRowDicColumn)
                    : [];
            ...
            rowDicRequests() {
                if (!this.rowDicColumns.size)
                    return [];
                return src_core_dic.getRowDicRequests(Array.from(this.rowDicColumns.values()), this.data, this.childrenKey);
            },
            isMobile() {
        ...
        methods: {
            ...
            handleLoadDic() {
                return this.runDicRequest('dic', () => src_core_dic.loadDic(this.resultOption, this, this.rowDicColumns.size > 0), (result) => {
                    this.handleSetDic(this.DIC, result);
                });
            },
            ...
            handleLoadRowDic(requests = this.rowDicRequests) {
                ...
                return this.runDicRequest('row-dic', () => Promise.all(requests.map((request) => {
                    ...
                    return (empty ? Promise.resolve([]) : src_core_dic.sendDic({
                        column,
                        value,
