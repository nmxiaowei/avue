...
var packages = (() => {
    return {
        logs(name) {
            ...
            if (!obj)
                return;
            src_plugin_logs_index.default.capsule(name, obj.title, 'warning');
            src_plugin_logs_index.default.warning('CDN:' + (obj.url || '-'));
            src_plugin_logs_index.default.warning('GITHUB:' + (obj.github || '-'));
        },
    };
})();
