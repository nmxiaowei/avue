...
export default {
  ...
  xlsx(file: File, options: Record<string, any> = {}) {
    ...
    return new Promise<{ header: any[]; results: any[]; sheetName: string }>((resolve, reject) => {
      const reader = new FileReader();
      const getHeaderRow = (sheet: any, rowIndex: number) => {
        const headers = [];
