<template>
  <div :class="b()">
    <div :class="b('canvas-wrap')">
      <canvas ref="canvas" :class="b('canvas')">当前浏览器不支持生成授权凭证。</canvas>
      <div v-if="loading" :class="b('loading')">正在生成授权凭证…</div>
      <div v-else-if="error" :class="b('error')">{{ error }}</div>
    </div>
    <div v-if="showStatus" :class="[b('status'), { 'is-ready': draw, 'is-error': error }]">
      <span v-if="loading">素材加载中</span>
      <span v-else-if="error">生成失败</span>
      <span v-else-if="draw">已生成，可导出图片或 PDF</span>
      <span v-else>等待生成</span>
    </div>
    <slot :ready="draw" :loading="loading" :error="error"></slot>
  </div>
</template>

<script>
import create from "core/create";
import { downFile as downloadFile } from "utils/util";
