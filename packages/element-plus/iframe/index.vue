<template>
  <div :class="b()" :style="styleName">
    ...
    <div v-if="!hasSrc" :class="b('state')">
      <slot name="empty">暂无嵌入页面</slot>
    </div>
    <div v-else-if="loadError || isTimedOut" :class="[b('state'), b('state-error')]">
      <slot name="error" :error="loadError" :timed-out="isTimedOut" :reload="reload">
        <span>{{ isTimedOut ? timeoutText : errorText }}</span>
        <button type="button" :class="b('retry')" @click="reload">重新加载</button>
      </slot>
    </div>
    <div v-else-if="loading && showLoading" :class="[b('state'), b('state-loading')]">
