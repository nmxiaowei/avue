<template>
  <div
    :class="[b(), { [b('bordered')]: bordered }]"
    :style="styleName"
    role="img"
    aria-label="二维码"
  >
    <canvas
      v-if="type === 'canvas' && qrcode"
      ref="canvas"
      :class="b('canvas')"
    ></canvas>
    <div
      v-else-if="type === 'svg' && qrcode"
      :class="b('svg')"
      v-html="svgContent"
    ></div>

    <div v-if="errorMessage" :class="[b('status'), b('status-error')]">
      {{ errorMessage }}
    </div>
    <div v-else-if="status !== 'active'" :class="b('status')">
      <span v-if="status === 'loading'" :class="b('loading')"></span>
      <template v-else-if="status === 'expired'">
        <span>二维码已过期</span>
        <button :class="b('refresh')" type="button" @click="refresh">
          点击刷新
        </button>
      </template>
...
export default create({
  ...
  methods: {
    updateQRCode() {
      try {
        const instance = qrcode(0, this.errorLevel);
