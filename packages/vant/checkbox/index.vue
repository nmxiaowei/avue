<template>
  <div class="avue-cell">
    <van-cell :title="label" :class="{'van-cell--required':required}" v-if="tags">
      <van-tag
        class="avue-cell__tags"
        plain
        v-for="(item,index) in dic"
        :type="text.includes(item[valueKey])?'primary':''"
        :key="index"
        @click.native="handleCheckboxClick(index)"
      >{{item[labelKey]}}</van-tag>
    </van-cell>
    <van-checkbox-group v-show="!tags" v-model="text" @change="handleChange" :max="max">
      <p :class="['avue-cell__title',{'avue-cell__title--required':required}]">
        {{label}}
        <small class="avue-cell__tip" v-if="tip">{{tip}}</small>
      </p>
      <van-cell-group>
        <van-cell
          v-for="(item,index) in dic "
